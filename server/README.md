...
- スコアはシート `scores` に 1 行ずつ追記されます (ts, stage, name, makespan, moves, paths…)。荒らしの行はシートで直接削除できます
  - 1 セルは 50,000 文字までなので、経路がそれを超える提出 (`ceo:500` は最大 7.6 万字) は F 列の右隣に続けて書かれます。ダンプ API は 6 列目以降を連結して 1 本の文字列に戻します
...
- **復旧の手順**: 新しいスプレッドシート + Apps Script を用意し、`scores` シートに `backups/scores-*.csv` を、
  `users` シートに `backups/users-*.csv` をそのまま貼り付ける (1 行目がヘッダー)。ts は数値、paths はカンマ区切りの文字列です。
  50,000 文字を超える paths (`ceo:500` など) は 1 セルに入らないので、F 列から右へ 45,000 文字ずつに分けて貼ってください (連結して読まれます)。
