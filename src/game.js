...
(function () {
  ...
  const I18N = {
    ja: {
      ...
      't4.h': '4. ゴールに着いたエージェントは動かない', 't4.p': '経路が終わったエージェントは、その後ずっとゴールに<b>留まり続けます</b>。そのマスを後から通ろうとすると頂点衝突です (まだ到着していないエージェントのゴールを通り抜けるのは OK)。先に着く人の場所を避けるか、順番を入れ替えましょう。参考解 (LNS2) もまったく同じルール (ゴール到着後は留まる・逆走禁止・待機と戻る動きは可) で解いています。',
    ...
    en: {
      ...
      't4.h': '4. Agents stay at their goal', 't4.p': 'Once an agent\'s path ends, it <b>stays at its goal forever</b>. Passing through that cell later is a vertex collision (passing through the goal of an agent that has not arrived yet is fine). Route around agents that arrive earlier, or change the order. The reference solution (LNS2) is computed under exactly the same rules (stay at goal, no head-on passing, waiting and turning back allowed).',
