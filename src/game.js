...
(function () {
  ...
  const I18N = {
    ja: {
      ...
      'map.manji': '卍', 'map.game': 'Game', 'map.pinwheel': 'Pinwheel', 'map.hundred': '100',
      // アカウント
      ...
      'notice.i3b': '9/11 8:00 時点でレーティング青以上の方のリストは<a href="https://x.com/fuwaorune/status/2098189681389400514?s=20" target="_blank" rel="noopener">こちら</a>です．',
      ...
      'rating.note': 'ステージごとの performance をまとめた値です。良い performance ほど重く、解いたステージが少ないうちは低めに出ます。performance は「ステージの difficulty」と「最良スコア (参考解と人間ベストの良い方) にどれだけ迫ったか」と「ステージ内の順位」から決まります。difficulty は LaCAM3 / LNS2 の計測値を出発点に、みなさんの回答状況で更新されます。',
      ...
      'maker.stageLimit': '1 マップに作成できるステージは 5 つまでです',
      'maker.tooManyStages': '1マップあたりに作成できるステージは5つまでです．',
    ...
    en: {
      ...
      'notice.i3b': 'The list of players rated blue or above as of 9/11 8:00 is <a href="https://x.com/fuwaorune/status/2098189681389400514?s=20" target="_blank" rel="noopener">here</a>.',
      ...
      'rating.doc': '📄 How diff & rating work',
      ...
      'rating.note': 'Your rating combines your per-stage performances: better ones weigh more, and it stays on the low side until you have solved a good number of stages. A performance depends on the stage difficulty, how close you got to the best known score (the better of the reference solution and the best human solution), and your rank on the stage. Difficulties start from LaCAM3 / LNS2 measurements and are updated by how players do.',
      ...
      'maker.stageLimit': 'Up to 5 stages per map.',
      'maker.tooManyStages': 'You can create at most 5 stages per map.',
      ...
      'desc.contest001': 'Human MAPF Contest 001',
      'desc.manji': 'writer: miku_hatsune', 'desc.game': 'writer: sigtuna', 'desc.pinwheel': 'writer: kukone',
      'desc.hundred': 'Celebrating 100 registered players.',
      't1.h': '1. Goal',
  ...
  function loadEmbeddedReference() {
    ...
    if (!R.paths) {
      // 経路なしの参考記録 (writer 採用ステージ: 作者の人力スコアのみ. 経路は投稿検証後に破棄されている)
      S.ref = { makespan: R.makespan, moves: R.moves }; S.refPaths = null; S.refState = 'ok';
      S.lb = R.lb;
      return true;
    }
  ...
  // 再生の開始. 未到達の経路があっても「衝突するか、全員が止まるまで」再生する (正解扱いにはしない).
  // paused=true はコマ送り用: t=0 で停止した状態から始める
  function startRun(paused) {
    if (S.mode !== 'edit') return;
    Sound.ensure();
    ...
    S.anim = { t: 0, last: performance.now(), failT, end: fail ? failT : S.metrics.makespan, arrived: new Set(), paths: S.paths, isRef: false, makespan: S.metrics.makespan,
      notDone: notDone.length ? notDone : null, paused: !!paused, finished: false };
    $('btn-judge').disabled = !paused; $('btn-stop').disabled = false; $('btn-edit-here').disabled = !paused;
  ...
  function animTick(now) {
    const a = S.anim; if (!a || S.mode !== 'play' || a.paused) return;
  ...
  // コマ送りで止めた時刻から経路を作り直す: 各経路を t 手までに切り詰めて編集モードへ (undo 可)
  ...
  function renderNotice() {
    ...
    el.innerHTML = `<h3>${t('notice.title')}<span class="date">${t('notice.date')}</span></h3>
      <p>${t('notice.lead')}</p>
      <ul>${['i1', 'i2', 'i3', 'i3b', 'i4', 'i5'].map(k => `<li>${t('notice.' + k)}</li>`).join('')}</ul>
...
${row('t6.drag', 't6.dragD')}${row('t6.click', 't6.clickD')}${row('t6.rclick', 't6.rclickD')}${row('t6.keys', 't6.keysD')}${row('t6.bs', 't6.bsD')}${row('t6.tab', 't6.tabD')}${row('t6.undo', 't6.undoD')}${row('t6.enter', 't6.enterD')}
  ...
  function mkAgentColor(i) { return `hsl(${(i * 137.508) % 360}, 70%, 45%)`; }
  ...
  function drawPlacements(c, cs, w, starts, goals) {
    ...
    goals.forEach((g, i) => {
      ...
      c.strokeStyle = mkAgentColor(i); c.lineWidth = 2; c.strokeRect(x + cs * 0.15, y + cs * 0.15, cs * 0.7, cs * 0.7);
      c.fillStyle = mkAgentColor(i); c.fillText(i + 1, x + cs / 2, y + cs / 2 + 1);
    });
    starts.forEach((s, i) => {
      const x = (s % w) * cs + cs / 2, y = ((s / w) | 0) * cs + cs / 2;
      c.fillStyle = mkAgentColor(i); c.beginPath(); c.arc(x, y, cs * 0.34, 0, Math.PI * 2); c.fill();
  ...
  function renderMakerStage(body) {
    ...
    body.innerHTML = `
      ...
      <div class="maker-note">${t('maker.stageLimit')}</div>
    ...
    $('ms-save').onclick = () => {
      ...
      m.stages = m.stages || {};
      // 上限 5 ステージ (server/maker.gs の MAKER.MAX_STAGES と合わせる). 既存ステージの上書きは可
      if (!m.stages[MK.stageN] && Object.keys(m.stages).length >= 5) { msg.className = 'maker-msg err'; msg.textContent = t('maker.tooManyStages'); return; }
