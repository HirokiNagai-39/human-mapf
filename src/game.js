...
(function () {
  ...
  const I18N = {
    ja: {
      ...
      't4.h': '4. 経路が終わったエージェントはそのマスに留まる', 't4.p': '各エージェントは経路の最後の時刻以降、最後のマス (ゴール) に<b>留まり続けます</b>。そのマスを後から通ろうとすると頂点衝突です。ゴールに着いた後にさらに動くことはできます (通り道を空けるなど。最終的にゴールに戻っていれば OK)。まだ経路が続いているエージェントのゴールを通り抜けるのも OK です。参考解 (LNS2) もまったく同じルール (経路終了後は留まる・逆走禁止・待機と戻る動きは可) で解いています。',
    ...
    en: {
      ...
      't4.h': '4. After its path ends, an agent stays where it is', 't4.p': 'After the last time step of its path, each agent <b>stays at its final cell (its goal)</b>. Passing through that cell later is a vertex collision. An agent may keep moving after reaching its goal (e.g. to make way), as long as it is back on the goal at the end. Passing through the goal of an agent whose path is still going is fine too. The reference solution (LNS2) is computed under exactly the same rules (stay put after the path ends, no head-on passing, waiting and turning back allowed).',
